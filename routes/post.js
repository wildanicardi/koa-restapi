...
const { postIndex, postStore, postUpdate, postDelete,postDetail,searchTitle,filterPost} = require("../controllers/PostController");

router.get("/api/posts",auth,postIndex);
router.post("/api/post",auth,postStore);
router.get("/api/post/search",auth,searchTitle);
router.get("/api/post/filter",auth,filterPost);
